...
import { BokehPass } from '~/assets/js/BokehPassMine.js';
...
export default {
  ...
  methods: {
    ...
    initPostprocessing() {
      var self = this;
      var renderPass = new RenderPass(self.scene, self.camera);

      var bokehPass = new BokehPass(self.scene, self.camera, {
        focus: 1.0,
        aperture: 0.025,
        maxblur: 0.01,
        width: self.window.w,
      ...
      var composer = new EffectComposer(self.renderer);
      ...
      self.pixelRatio = self.renderer.getPixelRatio();
      self.fxaaPass.material.uniforms[ 'resolution' ].value.x = 1 / ( self.window.w * self.pixelRatio );
      self.fxaaPass.material.uniforms[ 'resolution' ].value.y = 1 / ( self.window.h * self.pixelRatio );

      composer.addPass(renderPass);
      composer.addPass(self.fxaaPass);
      composer.addPass(bokehPass);

      // composer = new EffectComposer( self.renderer );
      // composer.addPass( renderPass );
      // self.postprocessing.composer.addPass( self.fxaaPass );
    },
    createScene() {
      var self = this;
    ...
    animate() {
      ...
      var newMouse = {x: xPos, y: yPos}
      // console.log(newMouse)
      // console.log(newMouse)
      // Ease mouse movement - end
      ...
      this.raycaster.setFromCamera(newMouse, self.camera);
      ...
      var dist = self.distanceVector(intersects[0].point, self.camera.position)
